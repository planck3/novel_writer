...
class NovelWriter:
    def __init__(self, root):
        ...
        tab_width = self.font.measure('　' * 2)  # 使用全角空格计算宽度
        
        # 修改文本框配置
        self.text_content = tk.Text(
            self.root, 
            wrap=tk.CHAR,  # 修改为按字符换行
            undo=True, 
            yscrollcommand=scrollbar.set, 
            font=self.font, 
            tabs=(tab_width,),  # 修改制表符设置
            spacing1=5,  # 添加段落前间距
            spacing2=2,  # 添加段落间距
            spacing3=5   # 添加段落后间距
        )
        ...
        # 绑定Tab键事件
        self.text_content.bind('<Tab>', self.handle_tab)

        # 绑定空格键事件
        self.text_content.bind('<space>', self.handle_space)
        ...
        # 绑定光标移动事件，用于智能显示/隐藏按钮
        self.text_content.bind("<Motion>", self.check_cursor_position)
        self.text_content.bind("<ButtonRelease-1>", self.check_cursor_position)
        self.text_content.bind("<KeyRelease>", self.check_cursor_position)
        
        # 按钮隐藏状态标记
        self.button_hidden_by_cursor = False
        
    def handle_tab(self, event):
        """处理Tab键事件"""
        self.text_content.insert(tk.INSERT, '　' * 2)  # 插入两个全角空格
    ...
    def handle_space(self, event):
        ...
        try:
            ...
            # 如果前一个字符是ASCII字符或全角空格，让系统处理空格
            if prev_char.isascii() or prev_char == '　':
                return None
                
            # 如果前一个字符是中文字符，插入全角空格
            if ord(prev_char) > 255:  # 使用Unicode码点判断是否为中文字符
            ...
            logging.error(f"处理空格键时出错: {e}")
            return None  # 出错时让系统处理空格键
    ...
    def create_preview_button(self):
        ...
        # 将按钮移到右上角，避免遮挡文本内容
        self.preview_button.place(x=self.root.winfo_width() - 100, y=10)
        
        # 绑定窗口大小变化事件，确保按钮始终在右上角
    ...
    def check_cursor_position(self, event=None):
        """检查光标位置，决定是否显示预览按钮"""
        ...
        # 获取当前光标位置（相对于文本框）
        try:
            cursor_index = self.text_content.index(tk.INSERT)
            cursor_bbox = self.text_content.bbox(cursor_index)
            
            if not cursor_bbox:  # 如果光标不可见，则显示按钮
                if self.button_hidden_by_cursor:
                    self.preview_button.place(x=self.root.winfo_width() - 100, y=10)
                    self.button_hidden_by_cursor = False
                return
                
            # 将文本框坐标转换为窗口坐标
            text_x = self.text_content.winfo_x()
            text_y = self.text_content.winfo_y()
            cursor_x = text_x + cursor_bbox[0]
            cursor_y = text_y + cursor_bbox[1]
            
            # 定义右上角区域（按钮所在区域及其周围）
            button_region_x = self.root.winfo_width() - 120
            button_region_y = 40
            
            # 检查光标是否在按钮区域
            if cursor_x > button_region_x and cursor_y < button_region_y:
                # 光标在按钮区域，隐藏按钮
                if not self.button_hidden_by_cursor:
                    self.preview_button.place_forget()  # 暂时移除按钮
                    self.button_hidden_by_cursor = True
            else:
                # 光标不在按钮区域，显示按钮
                if self.button_hidden_by_cursor:
                    self.preview_button.place(x=self.root.winfo_width() - 100, y=10)
    ...
    def show_preview(self):
        ...
        if self.preview_window is None or not self.preview_window.winfo_exists():
            ...
            # 更新预览内容
            self.update_preview()
        elif self.preview_window.state() == 'withdrawn':
            # 如果窗口只是被隐藏了，重新显示它
    ...
    def save_file(self, event=None):
        ...
        # 读取开头部分文本
        # start_text = self.text_content.get("1.0", "1.0 + 2c").strip()  # 获取前两个字符
        # # 读取结尾部分文本，左开右闭
        # end_text = self.text_content.get("end - 3c", "end").strip()  # 获取最后两个字符
        
        all_text = self.text_content.get(1.0, tk.END).strip()
        all_text = all_text.replace(" ", "").replace("\n", "").replace("\t", "")
        # 从处理后的文本中获取前两个字符
        start_text = all_text[:2]
        # 从处理后的文本中获取最后两个字符
        end_text = all_text[-2:]
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"{start_text}-{current_date}-{end_text}.txt"  # 设置默认文件名
        )
        if not file_path:
            return
        try:
