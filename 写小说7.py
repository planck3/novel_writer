...
class NovelWriter:
    def __init__(self, root):
        ...
        self.preview_frame.pack(side=tk.LEFT, fill='y', padx=5, pady=5)

        # 创建预览画布
        self.preview_canvas = tk.Canvas(self.preview_frame, width=200, height=300, bg='#FFE4E1')
        self.preview_canvas.pack(pady=10)

        # 创建右侧编辑框架
        self.edit_frame = ttk.Frame(self.main_frame)
        self.edit_frame.pack(side=tk.LEFT, expand=True, fill='both', padx=5, pady=5)

        # 创建垂直滚动条
        scrollbar = tk.Scrollbar(self.edit_frame)
        ...
        self.text_content = tk.Text(self.edit_frame, wrap=tk.WORD, undo=True, yscrollcommand=scrollbar.set, font=self.font, tabs=tab_width)
    ...
    def scroll_to_end(self, event):
        ...
        # 添加翻页动画
        self.animate_page_turn()
        
    def draw_book_preview(self):
        """绘制书籍预览效果"""
        # 清除画布
        ...
        self.preview_canvas.create_polygon(
            40, 50, 160, 50,  # 顶边
            160, 250, 40, 250,  # 底边
            fill='white', outline='#8B0000', width=2
        )
        ...
        self.preview_canvas.create_polygon(
            160, 50, 180, 60,  # 右上角
            180, 260, 160, 250,  # 右边
            fill='#FFE4E1', outline='#8B0000', width=1
        )
        
        # 页面纹理
        for y in range(70, 240, 20):
            self.preview_canvas.create_line(
                50, y, 150, y,
                fill='#DEB887', dash=(2, 4)
            )
        ...
        try:
            ...
            if text_content:
                # 创建一个较小的字体用于预览
                preview_font = tkfont.Font(family=self.font.cget("family"), size=10)
                
                # 计算每行能显示的字符数
                chars_per_line = 10  # 根据书本宽度调整
                
                # 分割文本为多行
                lines = []
                for i in range(0, len(text_content), chars_per_line):
                    if i + chars_per_line < len(text_content):
                        lines.append(text_content[i:i+chars_per_line])
                    else:
                        lines.append(text_content[i:])
                
                # 限制显示的行数
                max_lines = 8  # 根据书本高度调整
            ...
            print(f"在预览中显示文本时出错: {e}")

    def animate_page_turn(self):
        """创建翻页动画效果"""
        def update_animation(step):
            self.preview_canvas.delete("page")
            # 绘制翻页动态效果
            curve = [(160, 50)]
            for i in range(5):
                x = 160 + (step * i / 4)
                y = 50 + (i * 50)
                curve.append((x, y))
            curve.append((160, 250))
            
            self.preview_canvas.create_polygon(
                *[coord for point in curve for coord in point],
                fill='#FFE4E1', outline='#8B0000',
                width=1, tags="page"
            )
            
            if step < 20:
                self.root.after(50, lambda: update_animation(step + 1))
            else:
                self.draw_book_preview()
